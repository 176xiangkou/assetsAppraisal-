<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="priceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="价格编码" align="center" prop="id" />
...
<script>
import { getAllPrice, listPrice, getPrice, delPrice, addPrice, updatePrice, exportRole, dataScope, changeRoleStatus } from "@/api/project/price";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPrice(this.addDateRange(this.queryParams, this.dateRange)).then(
        response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const priceIds = row.id || this.ids;
      this.$confirm('是否确认删除编号为"' + priceIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
